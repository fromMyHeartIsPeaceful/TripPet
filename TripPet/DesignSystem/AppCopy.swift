...
enum AppCopy {
    ...
    enum Health {
        ...
        static let localOnly = "Apple健康的步数只在本机用于判断是否可赠送机票，不会存入 iCloud。"
    ...
    enum Settings {
        ...
        static let iCloudTitle = "iCloud 同步"
        static let iCloudChecking = "正在检查 iCloud 状态"
        static let iCloudAvailable = "已连接 iCloud。游戏进度会保存在本机，并通过你的 iCloud 账户同步。"
        static let iCloudNoAccount = "未登录 iCloud。游戏进度会继续保存在本机，登录 iCloud 后可同步。"
        static let iCloudRestricted = "这台设备的 iCloud 同步受限制。游戏进度会继续保存在本机。"
        static let iCloudUnavailable = "iCloud 暂不可用。游戏进度已保存在本机，稍后会再次尝试同步。"
        static let iCloudErrorPrefix = "同步遇到问题"
